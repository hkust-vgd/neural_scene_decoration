...
def run_training(rank, world_size, model_args, data, split_file, load_from, new, num_train_steps, name, seed):
    is_main = rank == 0
    is_ddp = world_size > 1
    ...
    model.set_data_src(data, split_file)
...
def train_from_folder(
    ...
    if eval_fid:
        model = Trainer(**model_args)
        model.load(load_from)
        model.set_data_src(data, eval_mode=True)
        checkpoint = model.checkpoint_num
        num_batches = math.ceil(calculate_fid_num_images / batch_size)
        fid = model.calculate_fid(num_batches)
        print('FID Score for "{}" at checkpoint {}: {}'.format(name, str(checkpoint), fid))
        return
    ...
    if generate_unseen:
        model = Trainer(**model_args)
        model.load(load_from)
        model.set_inference_src(data)
        samples_name = timestamped_filename()
        checkpoint = model.checkpoint_num
        dir_result = model.generate(samples_name, num_image_tiles, checkpoint, generate_types, write_full=False)
        print(f'sample images generated at {dir_result}')
        return
    ...
    if aug_test:
        DiffAugmentTest(data=data, split_file=split_file, image_size=image_size, label_mode=label_mode,
                        resize_mode=resize_mode, batch_size=batch_size, types=aug_types, nrow=num_image_tiles)
        return

    world_size = torch.cuda.device_count()

    if world_size == 1 or not multi_gpus:
        run_training(0, 1, model_args, data, split_file, load_from, new, num_train_steps, name, seed)
        return

    mp.spawn(run_training,
        args=(world_size, model_args, data, split_file, load_from, new, num_train_steps, name, seed),
        nprocs=world_size,
        join=True)
