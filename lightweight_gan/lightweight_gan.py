...
from scenegen.dataset import SceneDataset, EvalDataset
from scenegen.discriminator import SceneDiscriminator
from scenegen.generator import SceneGenerator
...
def masked_mse_loss(real, fake, mask):
    losses = F.mse_loss(fake, real, reduction='none') * mask
    mean_loss = torch.sum(losses) / torch.sum(mask)
    return mean_loss
...
def evaluate_batch_in_chunks(max_batch_size, model, latent, label_batch, empty_image_batch):
    split_latent = latent.split(max_batch_size, dim=0)
    split_label_batch = label_batch.split(max_batch_size, dim=0)
    split_image_batch = empty_image_batch.split(max_batch_size, dim=0)

    chunked_outputs = [model(*i) for i in zip(split_latent, split_label_batch, split_image_batch)]
    if len(chunked_outputs) == 1:
        return chunked_outputs[0]
    return torch.cat(chunked_outputs, dim=0)
...
class ImageDataset(Dataset):
    def __init__(
        ...
        if split_file is not None:
            ...
            paths_fmt = '{}/data/{}/2D_rendering/{}/perspective/full/{}/rgb_rawlight.png'
            # empty_paths_fmt = '{}/data/{}/2D_rendering/{}/perspective/empty/{}/rgb_rawlight.png'
            self.paths = [Path(paths_fmt.format(self.folder, *record)) for record in paths_np]
        else:
...
class AugWrapper(nn.Module):
    ...
    def forward(self, images, labels, prob = 0., types = [], detach = False, **kwargs):
        context = torch.no_grad if detach else null_context

        with context():
            if random() < prob:
                images_with_labels = torch.cat((images, labels), dim=1)
                images_with_labels = random_hflip(images_with_labels, prob=0.5)
                images_with_labels = DiffAugment(images_with_labels, types=types)
                images, labels = torch.split(images_with_labels, [3, 10], dim=1)
    ...
    def __init__(
        ...
        *,
        ...
        D_kwargs = dict(
            ...
            fmap_inverse_coef=fmap_inverse_coef,
            attn_res_layers=attn_res_layers,
            use_label_disc=use_label_disc,
            label_mode=label_mode,
            _blur=Blur,
            _decoder=SimpleDecoder
        )

        self.G = SceneGenerator(**G_kwargs)
        self.D = SceneDiscriminator(**D_kwargs)

        self.ema_updater = EMA(0.995)
        self.GE = SceneGenerator(**G_kwargs)
...
class Trainer():
    ...
    def init_GAN(self):
        ...
        self.GAN = LightweightGAN(
            ...
            rank = self.rank,
            spade_skip=self.spade_skip,
            label_mode=self.label_mode,
            bg_cat=self.bg_cat,
            use_label_disc=(self.label_coeff is not None),
            *args,
            **kwargs
        )
    ...
    def set_data_src(self, folder, split_file=None, eval_mode=False):
        ...
        train_batch_size = math.ceil(self.batch_size / self.world_size)
        val_batch_size = self.num_image_tiles ** 2
        test_batch_size = 1 if eval_mode else self.batch_size

        train_dataloader = DataLoader(self.train_dataset, num_workers=num_workers, batch_size=train_batch_size,
                                      sampler=sampler, shuffle=(not self.is_ddp), drop_last=True, pin_memory=True)
        val_dataloader = DataLoader(self.val_dataset, num_workers=num_workers, batch_size=val_batch_size,
                                    sampler=None, shuffle=(not self.is_ddp), drop_last=True, pin_memory=True)
        test_dataloader = DataLoader(self.test_dataset, num_workers=num_workers, batch_size=test_batch_size,
                                     sampler=None, shuffle=False, drop_last=False, pin_memory=True)

        self.train_loader = cycle(train_dataloader)
        self.val_loader = cycle(val_dataloader)
        self.test_loader = cycle(test_dataloader)
    ...
    def set_inference_src(self, folder):
        self.test_dataset = EvalDataset(folder, split_file=self.split_file)
        test_dataloader = DataLoader(self.test_dataset, num_workers=1, batch_size=1, sampler=None, shuffle=False,
                                     drop_last=False, pin_memory=True)
        self.test_loader = cycle(test_dataloader)

    def train(self):
        assert exists(self.train_loader), 'You must first initialize the data source with `.set_data_src(<folder of images>)`'
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, G]):
            ...
            full_image_batch = image_dict['full_image'].cuda(self.rank)
            empty_image_batch = image_dict['empty_image'].cuda(self.rank)
            labels_batch = image_dict['objs_label'].cuda(self.rank)
            full_image_batch.requires_grad_()

            with amp_context():
                with torch.no_grad():
                    generated_images = G(latents, labels_batch, empty_image_batch)

                fake_output, fake_label_output, fake_output_32x32, _ = D_aug(
                    generated_images, labels_batch, detach=True, **aug_kwargs)

                real_output, real_label_output, real_output_32x32, real_aux_loss = D_aug(
                    full_image_batch, labels_batch, calc_aux_loss=True, **aug_kwargs)

                real_output_loss = real_output
                fake_output_loss = fake_output
                if self.label_coeff is not None:
                    real_output_loss += real_label_output * self.label_coeff
                    fake_output_loss += fake_label_output * self.label_coeff
            ...
            if apply_gradient_penalty:
                ...
                scaled_gradients = torch_grad(outputs=outputs, inputs=full_image_batch,
                                       grad_outputs=list(map(lambda t: torch.ones(t.size(), device = full_image_batch.device), outputs)),
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[G, D_aug]):
            ...
            with amp_context():
                ...
                epochs = (self.steps * batch_size * self.gradient_accumulate_every) / len(self.train_dataset)
                ...
                gen_loss = loss + recon_loss * self.recon_coeff
    ...
    def evaluate(self, num = 0, num_image_tiles = 4):
        ...
        image_dict = next(self.val_loader)
        empty_image_batch = image_dict['empty_image'].cuda(self.rank)
        labels_batch = image_dict['objs_label'].cuda(self.rank)

        # regular

        generated_images = self.generate_(self.GAN.G, latents, labels_batch, empty_image_batch)
        torchvision.utils.save_image(generated_images, str(self.results_dir / self.name / f'{str(num)}.{ext}'), nrow=num_rows)
        
        # moving averages

        generated_images = self.generate_(self.GAN.GE, latents, labels_batch, empty_image_batch)
        torchvision.utils.save_image(generated_images, str(self.results_dir / self.name / f'{str(num)}-ema.{ext}'), nrow=num_rows)

    @torch.no_grad()
    def generate(self, num=0, num_image_tiles=4, checkpoint=None, types=['default', 'ema'], write_full=True):
        self.GAN.eval()
        ...
        if 'ema' in types:
            for i in tqdm(range(num_image_tiles), desc='Saving generated EMA images'):
                ...
                generated_image = self.generate_(self.GAN.GE, latents, labels_batch, empty_image_batch)
                path = str(self.results_dir / dir_name / f'{str(num)}-{str(i)}-ema.{ext}')
                label_path = str(self.results_dir / dir_name / f'{str(num)}-{str(i)}-ema-label.{ext}')
                empty_path = str(self.results_dir / dir_name / f'{str(num)}-{str(i)}-ema-empty.{ext}')
                full_path = str(self.results_dir / dir_name / f'{str(num)}-{str(i)}-ema-full.{ext}')

                torchvision.utils.save_image(generated_image[0], path, nrow=1)
                torchvision.utils.save_image(labels_rgb[0], label_path, nrow=1)
                torchvision.utils.save_image(empty_image_batch[0], empty_path, nrow=1)
                if write_full:
                    torchvision.utils.save_image(full_image_batch[0], full_path, nrow=1)

                # Evaluation only
                label_npy_path = str(self.results_dir / dir_name / f'{str(num)}-{str(i)}-ema-label.npy')
                # label_tensor = F.avg_pool2d(labels_batch.squeeze(0), kernel_size=2, stride=2) > 0.5
                label_tensor = F.max_pool2d(labels_batch.squeeze(0), kernel_size=2, stride=2)
                label_tensor = label_tensor.type(torch.FloatTensor)
                np.save(label_npy_path, label_tensor.numpy())
    ...
    def calculate_fid(self, num_batches):
        ...
        if not real_path.exists() or self.clear_fid_cache:
            ...
            for batch_num in tqdm(range(num_batches), desc='calculating FID - saving reals'):
                image_dict = next(self.test_loader)
                real_batch = image_dict['full_image']
    ...
    @torch.no_grad()
    def generate_(self, G, style, labels_batch, empty_image_batch, num_image_tiles = 8):
        generated_images = evaluate_batch_in_chunks(self.batch_size, G, style, labels_batch, empty_image_batch)
